...
distance, idx = kdtree.query(R_obs)
best_params = lut_params[idx]
best_refl = lut_refl[idx]

print("✅ Nearest neighbor parameters:", best_params)
print("✅ Retrieval distance:", distance)

# === PLOT RETRIEVAL ===
plt.figure(figsize=(10, 5))
plt.plot(wavelengths, R_obs, 'k--', label='Observed')
plt.plot(wavelengths, best_refl, 'r-', label='Nearest Neighbor Fit')
plt.xlabel('Wavelength (nm)')
plt.ylabel('Reflectance')
plt.legend()
plt.grid(alpha=0.5)
plt.title('Nearest-Neighbor Reflectance Retrieval')
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'nearest_neighbor_retrieval.png'), dpi=200)
print("✅ Saved nearest neighbor reflectance comparison plot.")
